...
- `current_time`：当前时间，可指定 IANA timezone；`risk=read`。
- `calculate`：受限算术表达式求值，不执行 JS 代码；`risk=read`。
- `arxiv_search`：通过 arXiv public API 搜索论文；使用 `fast-xml-parser` 解析 Atom feed；`risk=read`。
- `github_search_repositories` / `github_get_repository` / `github_read_file`：通过 GitHub 官方 Octokit request 读取公开仓库和文件；可选 `GITHUB_TOKEN` 提升 rate limit；`risk=read`。
- `fetch_url`：读取 URL；`risk=external`，需要 `/approve`。
- `http_request`：curl-like HTTP 请求，支持 method/header/body，限制请求体和响应大小；`risk=external`，需要 `/approve`。
